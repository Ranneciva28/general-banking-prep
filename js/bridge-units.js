(() => {
  ...
  // Make the consolidated BRIDGE pool explicitly represent the newly uploaded
  // BRIDGE certification deck. The source deck contains the same full curriculum
  // sections, so the full version remains the cross-topic practice module.
  for(const q of bank){
    if(Number(q.moduleId)!==25)continue;
    q.moduleName='BRIDGE Module Full Version';
    q.category='BRIDGE Module';
    q.day=7;
    q.source=`${FULL_SOURCE} · Full Version`;
    q.bridgeFullVersion=true;
  }

  // Build 10 BRIDGE competency submodules from the aligned NUPMK competencies.
  // Each target keeps a separate identity/source and is deduped again by V28.
  for(const unit of units){
    for(let i=bank.length-1;i>=0;i--)if(Number(bank[i]?.moduleId)===unit.id)bank.splice(i,1);
    const base=bank.filter(q=>Number(q.moduleId)===unit.nupmkId);
    const additions=base.map((q,idx)=>({
      ...
      source:unit.source,
      bridgeSourceDeck:unit.source,
      bridgeUnit:true,
      bridgeUnitIndex:unit.id-35,
      bridgeMappedFromNupmk:unit.nupmkId,
  ...
  function family(value){
    ...
    const n=norm(t),first=n.split(' ')[0];
    if(first&&first.length>=6)return`prefix:${first}`;
    return'';
  }

  function hardenOptions(mid){
    const rows=bank.filter(q=>Number(q.moduleId)===mid&&Array.isArray(q.options)&&q.options.length===4);
    ...
    for(const q of rows){
      const f=family(q.answer),pool=f?groups.get(f):null;if(!pool||pool.length<4)continue;
      const candidates=pool.filter(x=>norm(x)!==norm(q.answer));
      const distractors=shuffle(candidates,`${mid}:${q.id}:bridge-plausible`).slice(0,3);if(distractors.length!==3)continue;
      q.options=shuffle([clean(q.answer),...distractors],`${mid}:${q.id}:bridge-options`);
      q.answer=clean(q.answer);q.distractorQuality='same-family-bridge';
    }
  }
  hardenOptions(25);for(const u of units)hardenOptions(u.id);

  // Day 1–5 are only source material now. Once NUPMK + BRIDGE are constructed,
  // remove those source modules from runtime to keep the browser lighter.
  for(let i=bank.length-1;i>=0;i--){const mid=Number(bank[i]?.moduleId);if(mid>=1&&mid<=24)bank.splice(i,1);}

  window.BRIDGE_UNITS=units.map((u,i)=>({...u,index:i+1,moduleName:`BRIDGE · ${u.name}`,day:7,maxBank:500}));
  window.BRIDGE_FULL={id:25,name:'BRIDGE Module Full Version',source:FULL_SOURCE,maxBank:500};
