(() => {
  ...
  try{const mk='gbpHotsSelectorV20';if(!localStorage.getItem(mk)){localStorage.removeItem('gbpDbBankV14');localStorage.removeItem(STATE_KEY);state={};localStorage.setItem(mk,'1')}}catch(e){}

  function question(mid,slot){
    const p=pool(mid),base=p[Number(slot)-1];if(!base)return null;
    return {...base,id:base.id||`V20-M${mid}-S${slot}`,bankSlot:Number(slot),bankEpoch:20,generated:true,baseId:base.rootQuestionId||base.baseId||base.id,structureKey:base.conceptSignature||`${rootKey(base)}|${base.variantMode||''}|${semanticKey(base)}`};
  }
  function validUnique(q,seenStem,seenOpt,seenSem){
    const sk=textKey(q),ok=optionKey(q),ck=semanticKey(q);
    return !!(q&&sk&&ok&&ck&&!seenStem.has(sk)&&!seenOpt.has(ok)&&!seenSem.has(ck)&&Array.isArray(q.options)&&q.options.length===4&&q.options.some(x=>clean(x)===clean(q.answer)));
  }
  function buildSet(mid,{respectHistory=true,seedOffset=0}={}){
    const p=pool(mid),active=[],seenStem=new Set(),seenOpt=new Set(),seenSem=new Set(),rootCounts=new Map();
    if(!p.length)return active;
    ...
    const scan=(maxRoot,ignoreHistory,ignoreRootHistory)=>{
      for(let i=0;i<p.length*2&&active.length<ACTIVE_LIMIT;i++){
        const slot=((start+i*step)%p.length)+1,q=question(mid,slot);if(!q||!validUnique(q,seenStem,seenOpt,seenSem))continue;
        const sk=textKey(q),ok=optionKey(q),ck=semanticKey(q),rk=rootKey(q);
        if((rootCounts.get(rk)||0)>=maxRoot)continue;
    ...
    scan(1,false,false);if(active.length<ACTIVE_LIMIT)scan(1,false,true);if(active.length<ACTIVE_LIMIT)scan(2,false,true);if(active.length<ACTIVE_LIMIT)scan(2,true,true);if(active.length<ACTIVE_LIMIT)scan(3,true,true);
    return active.slice(0,ACTIVE_LIMIT);
  }
  function replaceModule(mid,slots){
    const target=window.QUESTION_BANK||[],others=target.filter(q=>Number(q.moduleId)!==Number(mid)),seenStem=new Set(),seenOpt=new Set(),seenSem=new Set(),qs=[];
    for(const slot of slots){const q=question(mid,slot);if(!validUnique(q,seenStem,seenOpt,seenSem))continue;seenStem.add(textKey(q));seenOpt.add(optionKey(q));seenSem.add(semanticKey(q));qs.push(q)}
  ...
  async function ensureModule(mid){
    ...
    if(st.active.length===ACTIVE_LIMIT){const seenStem=new Set(),seenOpt=new Set(),seenSem=new Set();let valid=true;for(const slot of st.active){const q=question(mid,slot);if(!validUnique(q,seenStem,seenOpt,seenSem)){valid=false;break}seenStem.add(textKey(q));seenOpt.add(optionKey(q));seenSem.add(semanticKey(q))}if(valid){replaceModule(mid,st.active);if(!st.synced)setTimeout(()=>syncActive(mid,st.active),0);return st.active}}
  ...
  applyPreviews();
  document.addEventListener('DOMContentLoaded',()=>{if(window.GBPQuestionBank){window.GBPQuestionBank.generate=mid=>generate(mid,document.querySelector('.quiz-generate-btn'));window.GBPQuestionBank.bankInfo=mid=>{const st=getState(mid);return{active:st.active.length||ACTIVE_LIMIT,database:true,engine:'v20-hots-direct'}}}window.GBPDatabaseQuestionBank={engine:'v20-hots-direct',bankSize:Math.max(...moduleIds.map(mid=>pool(mid).length),0),ensureModule,reserveNew,clientId}});
})();