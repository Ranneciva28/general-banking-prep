(() => {
  ...
  function nearDuplicate(a,b,threshold=.76){
    ...
    const ea=semanticKey(a),eb=semanticKey(b);
    if(ea&&eb&&ea===eb&&(String(a.conceptSignature||'').startsWith('material-concept:')||String(b.conceptSignature||'').startsWith('material-concept:')))return true;
  ...
  try{const mk='gbpDistinctStructureSelectorV26';if(!localStorage.getItem(mk)){['gbpDbBankV14','gbpDbBankV20','gbpDbBankV24','gbpDbBankV25','gbpQuestionSeenV20','gbpQuestionSeenV24','gbpQuestionSeenV25'].forEach(k=>localStorage.removeItem(k));localStorage.removeItem(STATE_KEY);state={};localStorage.setItem(mk,'1')}}catch(e){}

  function question(mid,slot){const p=pool(mid),base=p[Number(slot)-1];if(!base)return null;return {...base,id:base.id||`V26-M${mid}-S${slot}`,bankSlot:Number(slot),bankEpoch:26,generated:true,baseId:base.rootQuestionId||base.baseId||base.id,structureKey:structureKey(base)};}
  ...
  function buildSet(mid,{respectHistory=true,seedOffset=0,excludeSlots=[],excludeQuestions=[]}={}){
    const p=pool(mid),active=[],chosen=[],chosenRoots=new Set(),chosenStructures=new Set();if(!p.length)return active;
    ...
    const excludedStructures=new Set(excluded.map(structureKey).filter(Boolean));
    ...
    const scan=(ignoreHistory,threshold)=>{
      for(const slot of slots){
        ...
        const rk=rootKey(q),sk=textKey(q),ck=semanticKey(q),stk=structureKey(q);
        if(chosenRoots.has(rk)||excludedRoots.has(rk)||chosenStructures.has(stk)||excludedStructures.has(stk))continue;
        if(!ignoreHistory&&respectHistory&&(localSeen['q:'+sk]||localSeen['c:'+ck]||localSeen['r:'+rk]||localSeen['s:'+stk]))continue;
        if(!canAdd(q,chosen,excluded,threshold))continue;
        chosenRoots.add(rk);chosenStructures.add(stk);chosen.push(q);active.push(slot);
      }
    };
    scan(false,.74);
    if(active.length<ACTIVE_LIMIT)scan(true,.74);
    if(active.length<ACTIVE_LIMIT)scan(true,.82);
  ...
  function validateSlots(mid,slots){
    ...
    const chosen=[];
    for(const slot of slots){const q=question(mid,slot);if(!canAdd(q,chosen,[],.82))return false;chosen.push(q);}
    return true;
  }
  function replaceModule(mid,slots){
    ...
    for(const slot of slots){const q=question(mid,slot);if(!canAdd(q,chosen,[],.82))continue;chosen.push(q);qs.push(q);}
  ...
  function previewSlots(mid){
    ...
    if(slots.length!==ACTIVE_LIMIT)console.error(`Module ${mid} strict bank only ${slots.length}/${ACTIVE_LIMIT}`,{pool:pool(mid).length});
    return slots;
  }
  function applyPreviews(){
    ...
    for(const mid of moduleIds){
      ...
      if(slots.length===ACTIVE_LIMIT){for(const slot of slots){const q=question(mid,slot);if(q)next.push(q);}continue;}
      const fallback=original.filter(q=>Number(q.moduleId)===Number(mid)).slice(0,ACTIVE_LIMIT);
      if(fallback.length)next.push(...fallback);
    }
  ...
  function markSeen(mid,active){for(const slot of active){const q=question(mid,slot);if(!q)continue;const sk=textKey(q),ck=semanticKey(q),rk=rootKey(q),stk=structureKey(q),now=Date.now();if(sk)localSeen['q:'+sk]=now;if(ck)localSeen['c:'+ck]=now;if(rk)localSeen['r:'+rk]=now;if(stk)localSeen['s:'+stk]=now;}saveSeen();}
  ...
  function pickLocalUnique(mid,{replaceCurrent=false}={}){
    ...
    if(replaceCurrent){
      ...
      for(const q of newQuestions)for(const old of previousQuestions)if(nearDuplicate(q,old,.78))throw new Error(`regeneration-overlap-${mid}`);
    }
  ...
  async function generate(mid,button){
    ...
    try{
      ...
      location.reload();
    }catch(e){console.error(e);setBusy(button,false);toast('Belum tersedia 25 soal baru yang benar-benar berbeda secara substansi untuk mengganti set ini.');}
  }
