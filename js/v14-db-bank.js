(() => {
  ...
  function baseFor(mid,slot){
    ...
    const seed=hash(`base-v14:${mid}:${slot}`);
  ...
  function question(mid,slot){
    ...
    return {...q,id:`DB-M${mid}-V14-S${slot}`,source:`${base.source} · DB Bank ${slot}/${BANK_SIZE}`,generated:true,bankSlot:slot,bankEpoch:1,baseId:base.id,isLong:isLongSlot(slot),structureKey};
  }
  ...
  applyPreviews();
  document.addEventListener('DOMContentLoaded',()=>{if(window.GBPQuestionBank){window.GBPQuestionBank.generate=mid=>generate(mid,document.querySelector('.quiz-generate-btn'));window.GBPQuestionBank.bankInfo=mid=>{const st=getState(mid);return{active:st.active.length||50,seen:null,remaining:null,database:true,engine:'v14'}}}window.GBPDatabaseQuestionBank={engine:'v14',bankSize:BANK_SIZE,ensureModule,reserveNew,clientId}});
})();