(() => {
  ...
  const cleanQuestion=text=>String(text||'').replace(/\s+/g,' ').trim();
  ...
  function professionalStem(base,seed){
    const q=cleanQuestion(base.question);
    const style=Math.floor(seeded(seed,3)*10);
    if(style<=3) return q;
    if(style===4) return `Perhatikan pernyataan berikut. ${q}`;
    if(style===5) return `${q} Pilih jawaban yang paling tepat.`;
    if(style===6) return `Berdasarkan kondisi berikut, tentukan jawaban yang paling tepat. ${q}`;
    if(style===7) return `Manakah pilihan yang paling tepat untuk pertanyaan berikut? ${q}`;
    if(style===8) return `Cermati kasus berikut. ${q}`;
    return `Jika ${lowerFirst(q)}, manakah kesimpulan atau tindakan yang paling tepat?`;
  }
  ...
  function bankQuestion(mid,slot,epoch){
    ...
    return {
      ...
      question:professionalStem(base,seed),
  ...
  function applyActivePool(){
    ...
    moduleIds.forEach(mid=>{
      ...
      if(st.active.length){
        ...
        next.push(...basePool(mid).slice(0,ACTIVE_LIMIT));
      }
    });
  ...
  function pickFresh(st,count){
    const seen=new Set(st.seen),picked=[];
    let guard=0;
    while(picked.length<count&&guard<50000){
      const n=1+Math.floor(Math.random()*BANK_SIZE);guard++;
      if(!seen.has(n)&&!picked.includes(n))picked.push(n);
    }
    if(picked.length<count){
      for(let n=1;n<=BANK_SIZE&&picked.length<count;n++)if(!seen.has(n)&&!picked.includes(n))picked.push(n);
    }
    return picked;
  }
  ...
  function generate(mid){
    ...
    sessionStorage.setItem('gbpGenerationToast','50 soal baru sudah aktif. Progress modul direset dan soal lama diganti.');
    location.reload();
  }
    ...
    try{
      await fetch(`${SUPA_URL}/rest/v1/rpc/gbp_track_event`,{
        ...
        body:JSON.stringify({
          ...
          p_latitude:locationCache?.latitude??null,p_longitude:locationCache?.longitude??null,
  ...
  function injectQuizGenerator(){
    ...
    const card=quiz.querySelector('.module-info-card');
    const mid=currentModuleId();
    if(!card||!mid)return;
    const info=bankInfo(mid);
    let box=card.querySelector('.quiz-generate-box');
    if(!box){box=document.createElement('div');box.className='quiz-generate-box';card.appendChild(box)}
    const sig=`${mid}:${info.seen}:${info.remaining}`;
    ...
    box.querySelector('button').onclick=()=>{
      if(confirm('Generate 50 soal baru? Progress pengerjaan modul ini akan direset dan soal aktif sekarang akan diganti.'))generate(mid);
    };
  }

  function inject(){cleanUI();injectQuizGenerator()}
  ...
  try{
    navigator.permissions?.query({name:'geolocation'}).then(p=>{
      if(p.state==='granted')navigator.geolocation?.getCurrentPosition(pos=>{
        locationCache={latitude:Number(pos.coords.latitude.toFixed(5)),longitude:Number(pos.coords.longitude.toFixed(5))};track('heartbeat');
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    inject();
    const msg=sessionStorage.getItem('gbpGenerationToast');
    if(msg){sessionStorage.removeItem('gbpGenerationToast');setTimeout(()=>toast(msg),450)}
    const auto=Number(sessionStorage.getItem('gbpAutoOpenModule'));
    if(auto){
      sessionStorage.removeItem('gbpAutoOpenModule');
      setTimeout(()=>window.GBPApp?.startModule?.(auto),120);
    }
