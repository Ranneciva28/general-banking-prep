(() => {
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    const key='gbpCacheNotice-20260822-1036';if(sessionStorage.getItem(key))return;
    ...
    wrap.querySelector('#clearCacheBtn')?.addEventListener('click',async e=>{const b=e.currentTarget;b.disabled=true;b.textContent='Clearing cache…';await clearQuestionCache();sessionStorage.setItem(key,'1');cleanLegacyCacheParams();location.reload();});
  },{once:true});
